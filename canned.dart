...
class Canned extends StatefulWidget {
  const Canned({super.key});

  @override
  _ProductsState createState() => _ProductsState();
}

class _ProductsState extends State<Canned> {
  ...
  void _filterProducts() {
    ...
    setState(() {
      ...
      List<Map<String, dynamic>> combinedProducts = List.from(products);
  ...
  void _editPrice(Map<String, dynamic> product) async {
    ...
    if (newPrice != null) {
      setState(() {
        ...
        int hiveIndex = productsBox1.values.toList().indexWhere(
          (item) => item["name"] == product["name"],
        );
        if (hiveIndex != -1) {
          productsBox1.putAt(hiveIndex, product);
        }
      });
    }
  }
