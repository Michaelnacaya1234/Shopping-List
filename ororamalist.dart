...
class Ororamalist extends StatefulWidget {
  const Ororamalist({super.key});

  @override
  _ShoppingListState createState() => _ShoppingListState();
}

class _ShoppingListState extends State<Ororamalist> {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed:
            () => Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => OroramaPurchase()),
            ),
  ...
  void _moveToPurchaseList(int index) {
    ...
    if (index >= 0 && index < shoppingList.length) {
      setState(() {
        ...
        if (!masterList.any((item) => item["name"] == itemName)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("$itemName has been moved to purchase list"),
